...
> Reguläre Ausdrücke sind Gruppen von Zeichen oder Symbolen, die dafür verwendet werden, um ein spezifisches Muster innerhalb eines Textes zu finden.

Ein Regulärer Ausdruck ist ein Muster, das mit einem String von links nach rechts 
verglichen wird. Der Ausdruck "Regulärer Ausdruck" ist ein Zungenbrecher, man wird 
normalerweise die Abkürzungen "regex" oder "regexp" antreffen. Reguläre Ausdrücke 
werden dafür verwendet, um Text innerhalb eines Strings zu ersetzen, Fomate zu validieren,
Substrings basierend auf auf einem Muster aus einem String zu extrahieren und so viel mehr.

Stellen Sie sich vor, dass sie ein eAnwendung schreiben und eine Regel dafür erstellen
möchten, wenn ein Nutzer seinen Nutzernamen wählt. Wir wollen erlauben, dass der Nutzernamen
Buchstaben, Zahlen, Unterstriche und Bindestriche enthält. Wir möchten ebenfalls Zeichenanzahl
begrenzen, damit der Nutzername nicht hässlich uassieht. Wir verwenden den folgenden Regulären
Ausdruck um den Nutzernamen zu validieren:
...
Der oben stehende Reguläre Ausdruck kann die Strings `john_doe`, `jo-hn_doe` und
`john12_as` akzeptieren. Er findet nicht `Jo`, da de rString einen Großbichstaben
enthält und zu kurz ist.
...
Ein Regulärer ausdruck ist nur ein Muster von Zeichen das wir dazu verwenden, um 
ein Suche in einem Text durchzuführen. Der Reguläre Ausdruck `the` meint zum Beispiel:
...
Der Regulär Ausdruck `123` findet den String `123`. Der Reguläre Ausdruck wird mit dem
Eingabe-String  verglichen, indem jedes Zeichen in dem Regulären Ausdruck mit jedem Zeichen
in dem String verglichen wird, eins nach dem anderen. Reguläre Ausdrücke unterscheiden
nach Groß- unb dKleinschreibung, sodass der Reguläre Ausdruck `The`nicht den string `the`finden würde.
...
Metazeichen sind die Bausteine der Regulären Ausdrücke. Metazeichen stehen nicht für sich selbst,
sondern werden in einer besonderen Art interpretiert. Manche Metazeichen haben eine besondere Bedeutung
und werden in eckigen Klammern geschrieben. Die Metazeichen sind die folgenden:

|Meta character|Description|
|:----:|----|
|.|Punkt findet jedes einzelne Zeichen ausser einem line break.|
|[ ]|Zeichenklasse. Findet jedes Zeichen, das sich zwischen den beiden eckigen Klammern befindet.|
|[^ ]|Verneinte (negierte) Zeichenklasse. findet jedes Zeichen, das sich nciht zwischenden beiden eckigen Kalmmern befindet.|
|*|Findet 0 oder mehrere Wiederholungen des vorhergehenden Zeichens.|
|+|Findet 1 oder mehrere Wiederholungen des vorhergehenden Zeichens.
|?|Macht das vorhergehende Symbol optional.
|{n,m}|Klammern. Findet mindestens "n", aber nicht mehr als "m" Wiederholungen des vorhergehenden Zeichens.
|(xyz)|Zeichengruppe. Findet die Zeichen xyz in exakt dieser Reihenfolge.|
